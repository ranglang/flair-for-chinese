...
class WordEmbeddings(TokenEmbeddings):
    ...
    def __init__(self, embeddings):
        ...
        #添加中文的字向量支持
        if embeddings.lower() == 'zh':
            data_folder = os.path.join(os.getcwd(),'resources','w2v',"train.bin")
            print("reading w2v from {}".format(data_folder))
            print(data_folder)
            embeddings = data_folder
    ...
    def _add_embeddings_internal(self, sentences: List[Sentence]) -> List[Sentence]:
        ...
        for i, sentence in enumerate(sentences):
            ...
            for token, token_idx in zip(sentence.tokens, range(len(sentence.tokens))):
                ...
                if self.precomputed_word_embeddings.wv.__contains__(token.text):
                    word_embedding = self.precomputed_word_embeddings.wv.__getitem__(token.text)
